...
class TestInitBackendEndpoint:
    ...
    def test_init_backend_success(self, client: TestClient, temp_data_dir):
    ...
    def test_init_backend_idempotent(self, client: TestClient, temp_data_dir):
        """Test that init can be called multiple times safely."""
        # Call twice
        response1 = client.post("/backend/init")
        response2 = client.post("/backend/init")
...
class TestRemoveBackendEndpoint:
    ...
    def test_remove_backend_is_noop(self, client: TestClient, temp_data_dir):
        """Test that remove backend is a no-op (same as BigQuery)."""
        # Initialize first
        client.post("/backend/init")

        # Remove
        response = client.post("/backend/remove")
