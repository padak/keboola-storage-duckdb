"""Snapshot management endpoints: create, list, get, delete, restore (ADR-004)."""
...
def _validate_bucket_exists(project_id: str, bucket_name: str) -> None:
...
def _validate_table_exists(project_id: str, bucket_name: str, table_name: str) -> None:
    """Validate that table exists."""
    _validate_bucket_exists(project_id, bucket_name)
...
@router.post(
    ...
    description="Create a manual snapshot of a table.",
    dependencies=[Depends(require_project_access)],
)
async def create_snapshot(
    project_id: str, request: SnapshotCreateRequest
) -> SnapshotResponse:
    """Create a manual snapshot of a table."""
    _validate_table_exists(project_id, request.bucket, request.table)
...
@router.get(
    ...
    description="List snapshots for a project with optional filtering.",
    dependencies=[Depends(require_project_access)],
)
...
@router.get(
    ...
    description="Get detailed information about a snapshot including schema.",
    dependencies=[Depends(require_project_access)],
)
async def get_snapshot(project_id: str, snapshot_id: str) -> SnapshotDetailResponse:
    """Get detailed snapshot information."""
    _validate_project_exists(project_id)

    snapshot = metadata_db.get_snapshot_by_project(project_id, snapshot_id)
    if not snapshot:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={
...
@router.delete(
    ...
    description="Delete a snapshot and its files.",
    dependencies=[Depends(require_project_access)],
)
...
@router.post(
    ...
    description="Restore a table from a snapshot.",
    dependencies=[Depends(require_project_access)],
)
async def restore_snapshot(
    project_id: str, snapshot_id: str, request: SnapshotRestoreRequest
) -> SnapshotRestoreResponse:
    ...
    if not project_db_manager.bucket_exists(project_id, target_bucket):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={
                "error": "bucket_not_found",
                "message": f"Target bucket {target_bucket} not found",
                "details": {"project_id": project_id, "bucket_name": target_bucket},
            },
        )

    # Check if target table already exists (unless restoring to same location)
    if project_db_manager.table_exists(project_id, target_bucket, target_table):
    ...
    table_path = project_db_manager.get_table_path(project_id, target_bucket, target_table)
