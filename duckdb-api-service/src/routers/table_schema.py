...
==========================
All table schema operations are scoped to a branch via:
/projects/{id}/branches/{branch_id}/buckets/{bucket}/tables/{table}/...

Schema operations (add column, drop column, alter column, add/drop PK) are restricted
to the default branch only - dev branches inherit schema from main (Live View).
Row operations (delete rows, profile) work on both main and dev branches.
"""
...
def _validate_table_exists(
    ...
    """
    Validate that table exists in main project.
...
async def add_column(
    ...
    Note: Schema modifications are only allowed on the default branch.
    """
    ...
    require_default_branch(resolved_branch_id, "modify table schema")
    ...
    table_info = project_db_manager.get_table(
...
async def drop_column(
    ...
    if should_create_snapshot(
        resolved_project_id, bucket_name, table_name, "drop_column"
    ):
    ...
    try:
        ...
        metadata_db.log_operation(
            ...
            details={"column_name": column_name, "branch_id": branch_id},
            duration_ms=duration_ms,
        )
...
async def add_primary_key(
    ...
    try:
        ...
        metadata_db.log_operation(
            ...
            details={"columns": pk_request.columns, "branch_id": branch_id},
            duration_ms=duration_ms,
        )
...
async def drop_primary_key(
    ...
    try:
        ...
        metadata_db.log_operation(
            ...
            resource_id=f"{bucket_name}.{table_name}",
            details={
                "previous_primary_key": table_info["primary_key"],
                "branch_id": branch_id,
            },
            duration_ms=duration_ms,
        )
